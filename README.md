...
A 2025 campaign generated candidate molecules for pancreatic cancer and a clinical partner
asked for laboratory validation. Three molecules were sent to synthetic chemists.
Approximately one year of laboratory effort produced no compound.

The post-mortem found the delay was not only logistical. **None of the 73 generated
molecules passes a standard pre-synthesis filter, and none contains an aromatic ring**,
against 57 to 76% of purchasable compounds passing the same filter. The three structures actually sent to chemists
...
| G12R | 15% | 9XB7 (1.36 A) | validated; 1,663 of 10,000 screened, [resumable](RESUME-G12R.md) | cognate redock **0.99 A** |
| G12C | 1.7% | 8AFB | 2,000 | 82 hits, wrong variant for PDAC |

Screened coverage is **68%** of pancreatic KRAS by variant frequency. G12R is validated and partially screened; completing it takes coverage to **83%**.
...
| 2 | MM-GBSA rescoring | ~1.5 to 2.0 kcal/mol | ~5 min | **built** |
| 3 | MD pose stability (RMSD/RMSF) | kills false positives | 5 to 12 h | next, runs on an M1 Pro |
| 4 | FEP / TI binding free energy | **~1.0 kcal/mol** | 1 to 3 days | needs a real GPU |
...
**Built and gated:** eleven phases, four validated targets, three screens over 518,662
purchasable compounds, MM-GBSA rescoring, ADMET recalibrated against approved controls, and
a forward chemoselectivity check on top of retrosynthesis.

**Not built: Phase 10, laboratory capture.** Orders placed from inside the system, with NMR,
LC-MS, purity and assay readouts ingested back onto the molecule record that predicted them.
This matters more than anything above it, because every phase improves only if measured
outcomes return, and potency, cell response and exposure prediction cannot be calibrated
without assay data.

**No compound has been ordered or measured.** The pipeline selects candidates for
measurement; it does not establish that any of them work. Across roughly 13,000 compounds
docked against three variants, one beats the strongest clinical control, by 0.02 kcal/mol,
...
*What it caught:* ZINC tranche letters are **mass bins**, I = 425 to 450, J = 450 to 500,
K = 500 to 953. MRTX-1133 is 600 Da. The first library pulled was centred on 437 Da and had
...
Site-directed Vina into the validated box. One Vina per ligand at one core each, **4× faster** than one Vina on ten cores, because Vina's search threads poorly.
...
*What it caught, in this project's own code:* the first version re-embedded each ligand from SMILES, so
